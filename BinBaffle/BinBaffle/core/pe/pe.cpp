...
// read the `file` file from disk 
// and return the base address and the size of it
bool pe::ReadPeFile(std::string& file, PBYTE* pPe, size_t* sPe) {
	HANDLE	hFile				= INVALID_HANDLE_VALUE;
	PBYTE	pBuff				= NULL;
	DWORD	dwFileSize			= NULL,
			dwNumberOfBytesRead = NULL;

	printf("[i] Reading \"%s\" ... ", file.c_str());

	hFile = CreateFileA(file.c_str(), GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
