...
namespace memory {
    ...
    public:
        ...
        constexpr address offset(const std::ptrdiff_t offset = 0) const noexcept {
            if (address_ == 0U) {
                return *this;
            }
        ...
        template <typename Ty>
        std::expected<Ty, e_error_code> read() const {
        ...
        std::expected<T, e_error_code> deref() const {
        ...
        template <typename T = address>
        std::expected<T, e_error_code> get(std::size_t count = 1) const noexcept {
            if (!address_ || count == 0) {
        ...
        template <typename T = address>
        constexpr T* ptr(const std::ptrdiff_t offset = 0) const noexcept {
            return this->offset(offset).as<std::add_pointer_t<T>>();
        }

        template <typename T = address>
        constexpr T* self_inc_ptr(const std::ptrdiff_t offset = 0) noexcept {
        ...
        constexpr address align_down(const std::ptrdiff_t factor) const noexcept {
            return { address_ & ~(factor - 1U) };
        }

        constexpr address align_up(const std::ptrdiff_t factor) const noexcept {
        ...
        constexpr std::uintptr_t inner() const noexcept {
            return address_;
        }
